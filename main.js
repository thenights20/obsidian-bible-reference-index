...
function blockContainsReference(block, reference) {
...
var BibleIndexView = class extends import_obsidian.MarkdownRenderChild {
  ...
  buildLayout() {
    ...
    const controls = this.containerEl.createDiv({ cls: "bri-controls bri-controls-three-columns" });
    ...
    const renderSelectorOptions = () => {
      ...
      selectorOptions.createDiv({ cls: "bri-book-selector-home-divider" });
      ...
      for (const group of selectorGroups) {
        ...
        groupHeader.createSpan({ text: group.name, cls: "bri-book-selector-group-name" });
    ...
    const searchHeader = searchGroup.createDiv({ cls: "bri-search-header bri-search-header-responsive" });
    searchHeader.createEl("label", {
      text: "Pesquisar em todo o acervo",
      attr: { for: `${this.selectionKey}-search` }
    });

    const mode = searchHeader.createDiv({ cls: "bri-search-mode bri-search-mode-inline bri-search-mode-responsive" });
  ...
  renderResults() {
    ...
    } else if (this.selectedBook !== BIBLE_INDEX_HOME) {
  ...
  renderBookNavigation() {
    if (!this.resultsEl || this.selectedBook === BIBLE_INDEX_HOME) return;

    const currentIndex = BIBLE_BOOKS.findIndex((book) => book.name === this.selectedBook);
    const previousBook = currentIndex > 0 ? BIBLE_BOOKS[currentIndex - 1]?.name : "";
    const nextBook = currentIndex >= 0 && currentIndex < BIBLE_BOOKS.length - 1
      ? BIBLE_BOOKS[currentIndex + 1]?.name
      : "";

    const navigation = this.resultsEl.createDiv({ cls: "bri-book-navigation" });

    const backButton = navigation.createEl("button", {
      cls: "bri-book-navigation-back",
      attr: { type: "button", title: "Voltar para todos os livros da Bíblia" }
    });
    const backIcon = backButton.createSpan({ cls: "bri-book-navigation-button-icon" });
    (0, import_obsidian.setIcon)(backIcon, "arrow-left");
    backButton.createSpan({ text: "Voltar ao Índice Geral" });

    backButton.addEventListener("click", () => {
      this.selectedBook = BIBLE_INDEX_HOME;
      this.visibleCount = this.config.pageSize;
      this.selectionStore.set(this.selectionKey, BIBLE_INDEX_HOME);
      this.refreshData();
      window.requestAnimationFrame(() => {
        this.containerEl.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    ...
    const pager = navigation.createDiv({ cls: "bri-book-navigation-pager" });

    const previousButton = pager.createEl("button", {
      cls: "bri-book-navigation-adjacent bri-book-navigation-previous",
      attr: {
        type: "button",
        title: previousBook ? `Abrir ${previousBook}` : "Este é o primeiro livro"
      }
    });
    previousButton.disabled = !previousBook;
    const previousIcon = previousButton.createSpan({ cls: "bri-book-navigation-button-icon" });
    (0, import_obsidian.setIcon)(previousIcon, "chevron-left");
    const previousCopy = previousButton.createDiv({ cls: "bri-book-navigation-adjacent-copy" });
    previousCopy.createSpan({ text: "Livro anterior", cls: "bri-book-navigation-adjacent-label" });
    previousCopy.createSpan({ text: previousBook || "Nenhum", cls: "bri-book-navigation-adjacent-name" });
    if (previousBook) previousButton.addEventListener("click", () => this.openIndexBook(previousBook));

    const nextButton = pager.createEl("button", {
      cls: "bri-book-navigation-adjacent bri-book-navigation-next",
      attr: {
        type: "button",
        title: nextBook ? `Abrir ${nextBook}` : "Este é o último livro"
      }
    });
    nextButton.disabled = !nextBook;
    const nextCopy = nextButton.createDiv({ cls: "bri-book-navigation-adjacent-copy" });
    nextCopy.createSpan({ text: "Próximo livro", cls: "bri-book-navigation-adjacent-label" });
    nextCopy.createSpan({ text: nextBook || "Nenhum", cls: "bri-book-navigation-adjacent-name" });
    const nextIcon = nextButton.createSpan({ cls: "bri-book-navigation-button-icon" });
    (0, import_obsidian.setIcon)(nextIcon, "chevron-right");
    if (nextBook) nextButton.addEventListener("click", () => this.openIndexBook(nextBook));
  }
